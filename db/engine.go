...
func (d *DbEngine) NewNullString(s string) sql.NullString {
	if len(s) == 0 {
...
func (d *DbEngine) LoadCountyTable(data [][]string, c bool) error {
	...
	for _, row := range data {
		...
		query += "(?, ?, ?, ?, ?, ?, ?, ?, ?), "
		vals = append(vals, county_id, row[0], state_id, row[1], row[2], row[3], row[4], row[5], d.NewNullString(row[6]))
	}
...
func (d *DbEngine) loadLocalTaxPart(data [][]string, query string, pos int) error {
	...
	for _, row := range data {
		...
		vals = append(vals, fmt.Sprint(pos), row[0], county_id, row[2], d.NewNullString(row[3]), d.NewNullString(row[4]), d.NewNullString(row[5]), d.NewNullString(row[6]),
			d.NewNullString(row[7]), row[8], d.NewNullString(row[9]), d.NewNullString(row[10]),
			d.NewNullString(row[11]), d.NewNullString(row[12]), d.NewNullString(row[13]))
...
func (d *DbEngine) LoadStateTable(data [][]string, c bool) error {
	...
	for _, row := range data {
		query += "(?, ?, ?, ?, ?, ?, ?), "
		vals = append(vals, row[0], row[1], d.NewNullString(row[2]), d.NewNullString(row[3]), d.NewNullString(row[4]), d.NewNullString(row[5]), d.NewNullString(row[6]))
	}
...
func (d *DbEngine) LoadStateBracketTable(data [][]string, c bool) error {
	...
	for _, row := range data {
		query += "(?, ?, ?, ?, ?), "
		vals = append(vals, row[0], d.NewNullString(row[1]), d.NewNullString(row[2]), d.NewNullString(row[3]), d.NewNullString(row[4]))
	}
